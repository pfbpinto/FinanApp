...
CALL UpdateUserParentIncome (59,13,'Updated Name',500,'2025-04-01',FALSE,'')
BACKEND VISUALIZATION:
...
-- DELETE a UserAssetParentIncome
...
call DeleteUserAssetParentIncome (59,1,2,'')-- Para Criação de "Monthly" incomes

BACKEND VISUALIZATION:
  
select * from userfinancialactual where FinancialUserItemID=60
select * from u userfinancialforecast where FinancialUserItemID=60
...
BEGIN
    ...
    BEGIN 
    -- Primeiramente, temos que limpar toda referencia de childs relacionados ao Parent
        -- Criação de tabela temporária para armazenar IDs relacionados a forecasts
    ...
    IF v_ChildCheck IS NOT NULL THEN
        ...
        -- Criação de tabela temporária para armazenar IDs relacionados a actuals
        ...
        -- Deletar Childs da tabela UserFinancialActual
        DELETE FROM UserFinancialActual 
        WHERE UserFinancialActualID IN (SELECT UserFinancialActualID FROM TempActuals);
        
        -- Deletar Childs da tabela UserFinancialForecast
        DELETE FROM UserFinancialForecast 
        WHERE UserFinancialForecastID IN (SELECT UserFinancialForecastID FROM TempForecasts);

        -- Deletar Childs da tabela FinancialUserItem baseados na tabela temporária
    ...
    END IF;
        ...
        -- Deletar o Parent User Asset Income da relations
  			DELETE FROM UserForecastActualRelation
        ...
        -- Deletar o Parent User Asset Income da Forecast
        DELETE FROM UserFinancialForecast
        WHERE financialuseritemid =p_FinancialUserItemID;

        
        -- Deletar o Parent User Asset Income da Actual
        DELETE FROM FinancialUserItem 
        WHERE FinancialUserItemID = p_FinancialUserItemID;
    ...
    EXCEPTION 
        WHEN OTHERS THEN 
            ...
            -- Dropar tabelas temporárias para evitar resíduos
            DROP TABLE IF EXISTS TempForecasts;
            DROP TABLE IF EXISTS TempActuals;
